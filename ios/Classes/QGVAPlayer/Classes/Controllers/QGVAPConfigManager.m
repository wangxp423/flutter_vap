...
//    }
    ...
    AVAsset *asset = [AVAsset assetWithURL:[NSURL fileURLWithPath:_fileInfo.filePath]];
        CGSize size = [asset tracksWithMediaType:AVMediaTypeVideo].firstObject.naturalSize;
        NSInteger videoWidth = size.width;
        NSInteger videoHeight = size.height;
        double duration = _fileInfo.mp4Parser.duration;
        NSInteger fps = _fileInfo.mp4Parser.fps;
            NSDictionary *configDictionary = @{@"info" : @{
                @"aFrame":         @[
                    @0,
                    @0,
                    @(videoWidth/2),
